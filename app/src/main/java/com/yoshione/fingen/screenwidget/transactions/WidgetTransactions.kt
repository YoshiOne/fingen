...
class WidgetTransactions : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Telephony.Sms.Intents.SMS_RECEIVED_ACTION) {
            updateHandler.postDelayed({
                ...
                onUpdate(context, appWidgetManager, appWidgetManager.getAppWidgetIds(ComponentName(context, WidgetTransactions::class.java)))
            }, 1000)
        } else {
